...
class SubjectiveMap (map.Map):
    def __init__(self, w, h, knownPos):
        self._table = [[100 for x in range(w)] for y in range(h)]
        self._width = w
        self._height = h
    ...
    def getAdjacentTo(self, pos, knownPos):
        ...
        return [n if n in knownPos else None, \
                e if n in knownPos else None, \
                s if n in knownPos else None, \
                w if n in knownPos else None]
